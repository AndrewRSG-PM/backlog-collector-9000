...
function ActionButton({ label, onClick, disabled, variant = 'primary', className = '' }) {
  return (
    <button
      ...
      disabled={disabled}
      className={`
        px-5 py-2.5 text-xs tracking-wider font-bold border transition-all
        ${variant === 'primary'
...
function WorkflowCard({ title, description, workflowFile, inputs = {}, testMode = false }) {
  ...
  return (
    <div className="border border-[#2a2a2a] p-4 hover:border-[#333] transition-colors">
      <div className="flex items-start justify-between gap-4 mb-3">
        <div>
          <div className="text-sm font-bold text-white tracking-wide mb-1">
            {title}
            {testMode && <span className="ml-2 text-[10px] text-yellow-500 border border-yellow-500/40 px-1.5 py-0.5">TEST</span>}
          </div>
...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-8">
      {/* Date selector */}
      <div>
        <div className="text-xs text-[#555] tracking-wider mb-2">TARGET DATE</div>
        <div className="flex items-center gap-3">
          <input
            type="date"
            value={date}
            onChange={(e) => setDate(e.target.value)}
            className="bg-[#0d0d0d] border border-[#2a2a2a] text-[#e5e5e5] text-xs px-3 py-2 font-mono focus:outline-none focus:border-[#555]"
          />
          <button
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          >
            <div className="text-sm font-bold text-white tracking-wide mb-1 group-hover:text-[#ccc]">
              Assemble Backlog 2D ↗
            </div>
            <div className="text-xs text-[#555]">Відкриває Coda (кнопка 2D збірки). Make.com pipeline.</div>
          </a>
          <a
            href={CODA_3D_URL}
            target="_blank"
          ...
          >
            ...
            <div className="text-xs text-[#555]">Відкриває Coda (кнопка 3D збірки). Make.com pipeline.</div>
          </a>
        </div>
        <div className="text-[10px] text-[#333] mt-2 px-1">
